...
colGaucheSac.innerHTML=' <img src="./public/img/sac-objets-3g-g.png" alt="">'

let hautSac = document.createElement("div")
hautSac.classList.add("hautSac")
hautSac.innerHTML=' <img src="./public/img/potion_pkmn.png" alt="">'
...
//_______________CLASS POKEMON_________________
...
class Dracaufeu extends pokemoN{
    constructor(nom,pv,atk1Dracaufeu,atk2Dracaufeu,atk3Dracaufeu,atk4Dracaufeu){
        super(nom,pv)
...
function dracHasard() {
    let dracaufeuComp = [dracaufeu.CrocsFeu,dracaufeu.Flammèche,dracaufeu.LanceFlammes,dracaufeu.Tranche]
...
function finCombat() {
    if (mewtwo.pv<=0) {
        alert("Tu as perdu")
        document.location.href="./public/pages/accueil.html";
...
function condiPotion() {
    if (dracaufeu.pv <=20) {
        let audioPotion = document.createElement("div")
        containerGame.appendChild(audioPotion)
        var musiqCbt = document.querySelector(".musiqCbt")
        musiqCbt.pause()
        audioPotion.innerHTML=' <audio src="./public/audio/Out of mana.mp3" autoplay></audio>'
        if (!potion) {
        dracaufeu.pv += 30
        barreVieDracaufeu = `${dracaufeu.pv}%`
        alert("L'utilisateur a utilisé une potion ")
        potion = true;
